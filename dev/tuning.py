...
display_title()
def update(i):
    global step_size, max_capa, variance_list, capa_value, tuning_finish, prev_intercept
    if current_clamp:
    ...
    current_variance = var(V[:int((T0+T1+T2)/dt)])
    if (capa_button.valinit < max_capa) and tuning_finish == False:
         capa_button.valinit += step_size
         capa_button.reset()
         variance_list.append(current_variance)
         capa_value.append([capa_button.valinit])
         plt.subplot()

         plt.clf()
         plt.scatter(capa_value, 1 / array(variance_list), color = 'blue')

         if len(variance_list) > 1:
            regr = linear_model.LinearRegression()
            regr.fit(capa_value,variance_list)
            if prev_intercept == 0:
                prev_intercept = regr.intercept_
            plt.plot(capa_value, 1/array(regr.predict(capa_value)))
            predict_change = prev_intercept - regr.intercept_
            prev_intercept = regr.intercept_
            if regr.intercept_ - predict_change  < 0:
                tuning_finish = True
                print("Final capacitance value: ", capa_value[-1][0])   #Value before oscillation
                print("Calculated optimal value: ", 0.5*capa_value[-1][0])  #0.5* value before oscillation
            plt.plot()
